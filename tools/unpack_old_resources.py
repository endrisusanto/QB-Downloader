...
def unpack_resources(bin_path):
    ...
    print(f"Loaded old raw resources binary of size {len(data)} bytes")
    ...
    os.makedirs("/home/endri-pro/Videos/QuickBuild_Downloader/tools/extracted_old", exist_ok=True)
    ...
    while offset < len(data):
        ...
        output_path = f"/home/endri-pro/Videos/QuickBuild_Downloader/tools/extracted_old/{name}"
        with open(output_path, 'wb') as out_f:
            out_f.write(res_data)
            
        print(f"  Extracted: {name:<50} | Size: {res_size:<8} bytes")
        res_idx += 1

if __name__ == '__main__':
    unpack_resources("/home/endri-pro/Videos/QuickBuild_Downloader/tools/raw_resources_old.bin")
