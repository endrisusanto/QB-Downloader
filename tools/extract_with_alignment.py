# tools/extract_with_alignment.py
import struct
import os
...
def unpack_resources(bin_path):
    ...
    while offset < len(data):
        ...
        print(f"Index {res_idx}: offset=0x{offset:X}, read size={res_size}")
        offset += 4
        
        if offset + res_size > len(data):
            print(f"Error: resource size {res_size} at offset 0x{offset:X} exceeds remaining data size")
            break
