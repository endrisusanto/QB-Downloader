...
function AppContent() {
  ...
  function clearTerminal(key: "completed" | "failed") {
    const artifactIds = new Set(categoryRecord[key].flatMap((group) => group.artifacts.map((artifact) => artifact.id)));
    artifactIds.forEach((artifactId) => downloads.removeRow(artifactId));
    builds.setGroups((current) => current
