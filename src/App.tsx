...
import { Download, Flame, FolderOpen, Moon, RotateCcw, Settings, Sun, Wifi, WifiOff } from "lucide-react";
...
function AppContent() {
  ...
  const handleRemoteDownload = useCallback(
    async (qbId: string, artifactTypes: string[], autoStart: boolean = true) => {
      ...
      try {
        ...
        const prepared = { ...prepareGroup(normalized, filters, true), customFilters: filters };
  ...
  const handleRemoteCancelArtifact = useCallback((groupId: string, artifactId: string, pin: string) => {
    ...
    if (group && artifact) void downloads.cancelSingle(group, artifact);
    return Boolean(artifact);
  }, [builds.groups, settings.remoteCancelPin, downloads]);
  ...
  const handleRemotePauseGroup = useCallback((groupId: string) => {
    const group = builds.groups.find((g) => g.id === groupId);
    if (group) {
      void downloads.pause(group);
    }
  }, [builds.groups, downloads]);

  const handleRemotePauseArtifact = useCallback((groupId: string, artifactId: string) => {
    const group = builds.groups.find((g) => g.id === groupId);
    const artifact = group?.artifacts.find((a) => a.id === artifactId);
    if (group && artifact) {
      void downloads.pauseSingle(group, artifact);
    }
  }, [builds.groups, downloads]);

  const handleRemoteResumeGroup = useCallback((groupId: string) => {
    const group = builds.groups.find((g) => g.id === groupId);
    if (group) {
      void start(group);
    }
  ...
  const handleRemoteResumeArtifact = useCallback((groupId: string, artifactId: string) => {
    const group = builds.groups.find((g) => g.id === groupId);
    if (!group) return;
    const artifact = group.artifacts.find((a) => a.id === artifactId);
    if (artifact) {
  ...
  const { status: syncStatus } = useServerSync(
    ...
    { active: dataWaster.active, totalBytes: dataWaster.totalBytes, speedBps: dataWaster.speedBps },
    (action, concurrency, targetBytes) => {
      if (action === "start") {
        void dataWaster.start(concurrency, targetBytes, settings.serverUrl);
      } else {
        void dataWaster.stop();
      }
    },
    handleRemotePauseGroup,
    handleRemotePauseArtifact,
    handleRemoteResumeGroup,
    handleRemoteResumeArtifact,
  );
