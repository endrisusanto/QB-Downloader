...
function AppContent() {
  ...
  const { status: syncStatus } = useServerSync(
    ...
    (action, concurrency, targetBytes) => {
      if (action === "start") {
        void dataWaster.start(concurrency, targetBytes);
      } else {
