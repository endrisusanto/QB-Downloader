import { Check, ChevronsDownUp, ChevronsUpDown, Download, Trash2, X } from "lucide-react";
...
type Props = {
  ...
  onCancelAll: () => void;
  onClearTerminal: (key: "completed" | "failed") => void;
  onClearFetched?: () => void;
  ...
  onPauseArtifact?: (group: BuildArtifactGroup, artifact: Artifact) => void;
  onRemoveArtifact?: (groupId: string, artifactId: string) => void;
};
    ...
    <div className="accordion-stack">
      ...
      {(Object.keys(LABELS) as SectionKey[]).map((key) => {
        ...
        return <section className={`task-accordion ${props.sections[key] ? "open" : ""}`} key={key}>
          <div className="accordion-heading" role="button" tabIndex={0} onClick={() => props.onSection(key)}><span>{LABELS[key]}</span><div className="accordion-summary-actions" onClick={(event) => event.stopPropagation()}><button className="icon-button compact-icon" title={categoryExpanded ? `Collapse all ${LABELS[key].toLowerCase()}` : `Expand all ${LABELS[key].toLowerCase()}`} disabled={!groups.length} onClick={() => props.onToggleCategoryBuilds(key)}>{categoryExpanded ? <ChevronsDownUp size={16} /> : <ChevronsUpDown size={16} />}</button>{key === "fetched" && <><button className={`secondary-button compact selection-toggle ${allFetchedSelected ? "selected" : ""}`} disabled={!fetchedArtifacts.length} onClick={() => props.onToggleFetched(!allFetchedSelected)}><Check size={15} />{allFetchedSelected ? "Deselect all" : "Select all"}</button><button className="primary-button icon-only" title="Download selected" disabled={!selectedFetched} onClick={props.onDownloadFetched}><Download size={16} /></button><button className="secondary-button compact danger" disabled={!groups.length} title="Clear all fetched builds" onClick={props.onClearFetched}><Trash2 size={15} />Clear all</button></>}{key === "progress" && <button className="secondary-button compact danger" disabled={!groups.length} onClick={props.onCancelAll}><X size={15} />Cancel all</button>}{(key === "completed" || key === "failed") && <button className="secondary-button compact danger" disabled={!groups.length} onClick={() => props.onClearTerminal(key)}><Trash2 size={15} />Clear all</button>}</div><strong>{groups.length + (key === "fetched" ? props.loadingInputs.size : 0)}</strong></div>
          {props.sections[key] && <div className="group-list">{key === "fetched" && [...props.loadingInputs].map((input) => <div className="build-group loading" key={input}><div className="group-header"><span className="spinner" /><div><strong>{input}</strong><span>Fetching artifacts...</span></div></div></div>)}{groups.map((group) => <BuildGroup key={group.id} group={group} rows={props.rows} expanded={props.buildExpanded[group.id] ?? true} filters={group.customFilters || props.filters} readonlyCheckboxes={key !== "fetched"} onToggleExpanded={() => props.onBuildExpanded(group.id)} onToggleArtifact={(artifactId) => props.onToggleArtifact(group.id, artifactId)} onToggleAll={(selected) => props.onToggleGroup(group.id, selected)} onDownload={() => props.onDownload(group)} onCancel={() => props.onCancel(group)} onRetry={() => props.onRetry(group)} onRemove={() => props.onRemove(group)} onConfigureFilters={() => props.onConfigureFilters(group)} onDownloadArtifact={(artifact: Artifact) => props.onDownloadArtifact(group, artifact)} onPauseArtifact={(artifact: Artifact) => props.onPauseArtifact?.(group, artifact)} onRemoveArtifact={(artifactId) => props.onRemoveArtifact?.(group.id, artifactId)} />)}{groups.length === 0 && !(key === "fetched" && props.loadingInputs.size) && <div className="accordion-empty">No {LABELS[key].toLowerCase()}.</div>}</div>}
        </section>;
      })}
    </div>
  );
