...
export function writeDialogSnapshot(kind: DialogKind, group: BuildArtifactGroup, rows: Record<string, DownloadEvent>) {
  const key = dialogStorageKey(kind, group.id);
  localStorage.setItem(key, JSON.stringify({ kind, group, rows } satisfies DialogSnapshot));
